...
class JFShareItemCell: UICollectionViewCell {
    ...
    private func prepareUI() {
        ...
        shareIconImageView.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(layoutVertical(iPhone: 40))
            make.size.equalTo(CGSize(width: layoutVertical(iPhone: 50), height: layoutVertical(iPhone: 50)))
        }
        
        shareTitleLabel.snp.makeConstraints { (make) in
            make.centerX.equalTo(contentView)
            make.top.equalTo(shareIconImageView.snp.bottom).offset(layoutVertical(iPhone: 15))
        }
