//
//  SHPhotoDetailViewController.swift
//  SwiftNewProject
//
...
class SHPhotoDetailViewController: UIViewController {
    ...
    var model: JFArticleDetailModel? {
        didSet {
            ...
            //更新评论的数量
            if model?.plnum != "0" {
                self.bottomToolView.commentButton.setTitle(model!.plnum!, for: .normal)
            }
            //更新收藏状态
    ...
    func loadPhotoDetail(_ classid: Int, id: Int) {
        JFArticleDetailModel.loadNewsDetail(classid, id: id) { (articleDetailModel, error) in
            guard let model = articleDetailModel, error == nil else {
                return
            }
            self.model = model
    ...
    @objc fileprivate func didTappedRightBarButtonItem(_ item: UIBarButtonItem) -> Void {
        ...
        SHProgressHUD.showInfoWithStatus("举报成功，谢谢您的支持")
    }
    //准备UI
    @objc fileprivate func prepare() {
        view.backgroundColor = UIColor.init(red: 0.110 , green: 0.102, blue: 0.110, alpha: 1)
    ...
    fileprivate func updatebottomScrollViewConstraint() {
        ...
        if captionLabel.height > 50 {
            ...
            //重新约束高度 - 比滚动区域高度顶部高10 底部高30 加起来就是40
            bottomBgView.snp.updateConstraints({ (make) in
    ...
    fileprivate lazy var collectionView: UICollectionView = {
        ...
        collectionView.register(SHPhotoDetailCell.self, forCellWithReuseIdentifier: self.photoIdentifier)
    ...
    fileprivate lazy var navigationBarView: UIView = {
        ...
        rightButton.setTitle("举报", for: .normal)
        rightButton.setTitleColor(UIColor(red:0.545, green:0.545, blue:0.545, alpha:1), for: .normal)
        rightButton.addTarget(self, action: #selector(didTappedRightBarButtonItem(_:)), for: .touchUpInside)
    ...
    fileprivate lazy var captionLabel: UILabel = {
        ...
        captionLabel.textColor = UIColor.init(red: 0.945, green: 0.945, blue: 0.945, alpha: 1)
        captionLabel.numberOfLines = 0
        captionLabel.font = UIFont.systemFont(ofSize: 14)
        return captionLabel
    }()
    //底部工具条
    fileprivate lazy var bottomToolView: SHPhotoBottomBar = {
        let bottomToolView = Bundle.main.loadNibNamed("SHPhotoBottomBar", owner: nil, options: nil)?.last as! SHPhotoBottomBar
    ...
    fileprivate lazy var topTitleLabel: UILabel = {
        ...
        topTitleLabel.font = .systemFont(ofSize: 15)
        return topTitleLabel
    }()
    //MARK 分享的相关的方法暂时都没有实现
    // 分享的视图 暂时没有实现
...
extension SHPhotoDetailViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    //实现方法
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
         return photoModels.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: photoIdentifier, for: indexPath) as! SHPhotoDetailCell
        cell.delegate = self
...
extension SHPhotoDetailViewController: JFShareViewDelegate {
...
extension SHPhotoDetailViewController: SHPhotoBottomBarDelegate,JFCommentCommitViewDelegate {
    //点击事件
    //返回
    ...
    func didTappedCommentButton(_ button: UIButton) {
        //没有实现
        let commentVc = JFCommentTableViewController(style: .plain)
...
//            SHProgressHUD.showInfoWithStatus("没有分享的内容")
//            return
//        }
        //弹出分享的视图
    ...
    func didTappedCollectionButton(_ button: UIButton) {
        if JFAccountModel.isLogin() {
            ...
            JFNetworkTool.shareNetworkTool.post(ADD_DEL_FAVA, parameters: paramters, finished: { (status, result, tipString) in
                if status != .success {
                    return
                }
                ...
                if successResult["result"]["status"].intValue == 1 {
                    //增加成功
                    SHProgressHUD.showSuccessWithStatus("收藏成功")
                    button.isSelected = true
                } else if successResult["result"]["status"].intValue == 3{
                    //删除成功
                    SHProgressHUD.showSuccessWithStatus("取消收藏")
                    button.isSelected = false
                }
                sh_setupButtonSpringAnimation(button)
            })
        } else {
            SHProgressHUD.showWithStatus("你还没有登录，请登录之后再进行收藏")
        }
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + Double(Int64(1.0 * Double(NSEC_PER_SEC)))/Double(NSEC_PER_SEC), execute: {
            SHProgressHUD.dissmiss()
        })
    }
}
extension SHPhotoDetailViewController: SHPhotoDetailCellDelegate {
    //单击事件
    func  didOneTappedPhotoDetailView(_ scrollView: UIScrollView) {
        ...
        UIView.animate(withDuration: 0.25, animations: { 
            //状态栏
            UIApplication.shared.setStatusBarHidden(!UIApplication.shared.isStatusBarHidden, with: .slide)
    ...
    func didLongPressPhotoDetailView(_ scrollView: UIScrollView, currentImage: UIImage?) {
        ...
        let alertC = UIAlertController(title: "保存到相册", message: nil , preferredStyle: .alert)
        let save = UIAlertAction(title: "保存", style: .default) { (action) in
            UIImageWriteToSavedPhotosAlbum(image , self , #selector(SHPhotoDetailViewController.image(_:didFinishSvaingWithError:contextInfo:)), nil)
        }
    ...
    //保存到相册
    func image(_ image: UIImage, didFinishSvaingWithError error: NSError?, contextInfo: UnsafeRawPointer) {
        if let _ = error {
             SHProgressHUD.showInfoWithStatus("保存失败")
        } else {
             SHProgressHUD.showInfoWithStatus("保存成功")
        }
    }
}
